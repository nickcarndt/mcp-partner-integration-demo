...
// Create MCP handler for SSE transport
// This handles JSON-RPC 2.0 protocol: initialize, tools/list, tools/call
console.log('[MCP] SSE handler initialized');
...
export async function GET(request: Request): Promise<Response> {
  console.log('[MCP] GET request (SSE):', request.url);
...
export async function POST(request: Request): Promise<Response> {
  ...
  console.log('[MCP] POST request (SSE):', request.url);
...
export async function DELETE(request: Request): Promise<Response> {
  console.log('[MCP] DELETE request (SSE):', request.url);
  return handler(request);
}
